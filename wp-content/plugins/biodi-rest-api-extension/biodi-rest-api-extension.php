...
add_action('rest_api_init', function ()
{
    register_rest_route('v1', 'plant/(?P<id>\d+)', array(
        'methods' => 'GET',
        'callback' => 'listplant',
    ));
});
...
function listbalcony(WP_REST_Request $request){
    ...
    $results = $wpdb->get_results("SELECT arrangementsBalcony.*,plants.name AS 'nomPlante',plants.description AS 'descPlante',plants.img AS 'imgSrc' FROM balconies INNER JOIN arrangementsBalcony ON balconies.id = arrangementsBalcony._balconyId INNER JOIN plants ON arrangementsBalcony._contain = plants.id WHERE balconies.id=". $id, OBJECT);
...
/**
	Retrieve a plant
*/
function listplant(WP_REST_Request $request){
	global $wpdb;
    $id = $request->get_param('id');
    $results = $wpdb->get_results("SELECT * FROM plants WHERE id=".$id, OBJECT);
